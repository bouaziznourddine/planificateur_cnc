...
class Individual:
    ...
    def __init__(self, sequence: List[int], machine_assignments: List[int],
        ...
        # Structure des blocs [[OF_ids], [OF_ids], ...]
...
class GeneticAlgorithmScheduler:
    ...
    def _extract_of_data(self) -> Dict:
        ...
        for of in self.ofs:
            ...
            total_tools = sum(sum(op.outil_ids.mapped('quantite_requise'))
    ...
    def run(self) -> Tuple[Individual, Dict]:
        ...
        """
        _logger.info(
            f"🧬 Démarrage AG: Population={self.population_size}, Générations={self.generations}")
        ...
        _logger.info(
            f"Gen 0: Meilleur fitness = {best_individual.fitness:.2f} min (makespan)")
        ...
        _logger.info(
            f"✅ AG terminé: Meilleur makespan = {best_individual.makespan:.2f} min")
    ...
    def _initialize_population(self) -> List[Individual]:
        ...
        for _ in range(self.population_size):
            ...
            individual = Individual(
                sequence, machine_assignments, block_structure)
    ...
    def _evaluate_fitness(self, individual: Individual):
        ...
        for idx, block in enumerate(individual.block_structure):
            ...
            # Temps total du bloc
            block_duration = sum(
                self.of_data[of_id]['total_time'] for of_id in block)
            ...
            for of_id in block:
                if self.of_data[of_id]['date_livraison']:
                    ...
                    # À ajuster avec les vraies dates
    ...
    def _crossover(self, parent1: Individual, parent2: Individual) -> Tuple[Individual, Individual]:
        ...
        for i in range(max(len(child1_blocks), len(child2_blocks))):
            if random.random() < 0.5:
                m1 = parent1.machine_assignments[min(
                    i, len(parent1.machine_assignments)-1)]
                m2 = parent2.machine_assignments[min(
                    i, len(parent2.machine_assignments)-1)]
            else:
                m2 = parent1.machine_assignments[min(
                    i, len(parent1.machine_assignments)-1)]
                m1 = parent2.machine_assignments[min(
                    i, len(parent2.machine_assignments)-1)]
    ...
    def _mutate(self, individual: Individual):
        ...
        if mutation_type == 'swap' and len(individual.sequence) > 1:
            ...
            if len(individual.machine_assignments) != len(individual.block_structure):
                diff = len(individual.block_structure) - \
                    len(individual.machine_assignments)
                if diff > 0:
                    individual.machine_assignments.extend(
                        [random.randint(0, len(self.machines)-1) for _ in range(diff)])
                else:
                    individual.machine_assignments = individual.machine_assignments[:len(
                        individual.block_structure)]

        elif mutation_type == 'machine' and individual.machine_assignments:
            # Changer l'affectation d'un bloc
            block_idx = random.randint(
                0, len(individual.machine_assignments) - 1)
            individual.machine_assignments[block_idx] = random.randint(
                0, len(self.machines) - 1)
...
def create_gantt_chart_data(individual: Individual, of_data: Dict, machines: List,
    ...
    # Palette de couleurs pour les différents types de pièces
    colors = [
    ...
    for idx, block in enumerate(individual.block_structure):
        ...
        for of_id in block:
            ...
            # Déterminer la couleur basée sur le type de pièce
